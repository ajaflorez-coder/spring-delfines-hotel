...
public class ClienteRestController {
	...
	public ResponseEntity< List<Cliente> > fetchMedicos() {
		try {
			List<Cliente> medicos = clienteService.findAll();
			return new ResponseEntity<List<Cliente>>(medicos, HttpStatus.OK);   
		} catch (Exception e) {
	...
	public ResponseEntity<Cliente> deleteMedico(@PathVariable("id") Integer id) {
		try {			
			Optional<Cliente> optional = clienteService.findById(id);
			if(optional.isPresent()) {
	...
	@GetMapping(path = "/{id}/clientes")
	public ResponseEntity<List<Cliente>> fetchPaciente(
