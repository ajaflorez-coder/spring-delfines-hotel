...
public class ClienteRestController {
	...
	@Autowired
	private  AlquilerService alquilerSerive;
	...
	@GetMapping(path = "/{id}", 
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity< List<Cliente> > ClienteporId(
			@PathVariable("id") Integer id) {
		try {
			List<Cliente> cliente = clienteService.fetchporId(id);
	...
	@GetMapping(path = "/lugar/{lugar}", 
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity< List<Cliente> > ClienteporLugarNacimiento(
			@PathVariable("lugar") String lugar) {
		try {
			List<Cliente> cliente = clienteService.fetchporLugarnacimiento(lugar);
			return new ResponseEntity<List<Cliente>>(cliente, HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<List<Cliente>>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	public ResponseEntity< Alquiler > saveAlquiler(
		...
		try {
			...
			if(optional.isPresent()) {				
				...
				Alquiler nuevoAlquiler = alquilerSerive.save(alquiler);
