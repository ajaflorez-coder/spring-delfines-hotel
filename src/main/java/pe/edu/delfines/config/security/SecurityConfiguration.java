...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable();
		http
			// Agregando CORS a Spring Security
			.cors().and()
			// Policy 
			.authorizeRequests()
				.antMatchers("/api/clientes").permitAll()
				.antMatchers("/index.html").permitAll()
				.antMatchers("/cliente").authenticated() //.hasRole("ADMIN")
				.antMatchers("/cliente/nuevo").hasRole("ADMIN")
				.antMatchers("/cliente/edit/**").hasRole("ADMIN")
